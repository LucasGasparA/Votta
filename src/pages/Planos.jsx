...
export default function Planos() {
  ...
  const [exibirComparacao, setExibirComparacao] = useState(false)
  ...
  const rolarParaPlanos = () => plansRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <div className="inline-flex items-center gap-4 bg-white/10 rounded-2xl p-1.5">
            <button
              ...
              className={`px-5 py-2 rounded-xl text-sm font-semibold transition-all ${cicloCobranca === 'monthly' ? 'bg-white text-primary-800 shadow-md' : 'text-primary-200 hover:text-white'}`}
            >
              Mensal
            </button>
            <button
              onClick={() => setCicloCobranca('annual')}
              className={`flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold transition-all ${cicloCobranca === 'annual' ? 'bg-white text-primary-800 shadow-md' : 'text-primary-200 hover:text-white'}`}
            >
              Anual
              <AnimatePresence>
                {cicloCobranca === 'annual' ? null : (
                  <motion.span
          ...
          <div className="mt-8">
            <button onClick={rolarParaPlanos} className="inline-flex items-center gap-2 text-primary-200 hover:text-white text-sm transition-colors">
      ...
      <section ref={plansRef} className="px-6 -mt-16 pb-16 max-w-6xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
          {PLANS.map((plan, i) => {
            ...
            const showStrikethrough = cicloCobranca === 'annual' && plan.annualPrice !== null && plan.annualPrice > 0
      ...
      <section className="px-6 pb-16 max-w-5xl mx-auto">
        <button
          onClick={() => setExibirComparacao(v => !v)}
        ...
        >
          {exibirComparacao ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
          {exibirComparacao ? 'Ocultar comparação completa' : 'Ver comparação completa'}
        </button>
      ...
      <AnimatePresence>
        {exibirModal && planoSelecionado && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50"
            onClick={e => e.target === e.currentTarget && setExibirModal(false)}
          >
            <motion.div
              ...
              <div className="p-6 border-b border-primary-100">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="text-xl font-display font-bold text-primary-800">{planoSelecionado.name}</h3>
                  </div>
                  <button onClick={() => setExibirModal(false)} className="text-primary-300 hover:text-primary-600 p-1.5 rounded-lg hover:bg-primary-50">
                    <X size={20} />
                  </button>
                </div>
              </div>
              ...
              <div className="p-6 space-y-5">
                <div className="bg-primary-50 rounded-xl p-4">
                  <div className="flex items-baseline justify-between">
                    <span className="text-sm text-primary-500">Valor {cicloCobranca === 'annual' ? 'anual' : 'mensal'}</span>
                    <div className="text-right">
                      {cicloCobranca === 'annual' && planoSelecionado.monthlyPrice > 0 && (
                        <p className="text-xs text-primary-300 line-through">R$ {planoSelecionado.monthlyPrice}/mês</p>
                      )}
                      <p className="text-lg font-bold text-primary-800">
                        {planoSelecionado.monthlyPrice === 0
                          ? 'Grátis'
                          : `R$ ${cicloCobranca === 'annual' ? planoSelecionado.annualPrice : planoSelecionado.monthlyPrice}/mês`}
                      </p>
                    </div>
                  </div>
                  {cicloCobranca === 'annual' && planoSelecionado.monthlyPrice > 0 && (
                    <p className="text-xs text-green-600 font-medium mt-2">
                      Economia de R$ {(planoSelecionado.monthlyPrice - planoSelecionado.annualPrice) * 12}/ano
                    </p>
                  )}
                </div>
